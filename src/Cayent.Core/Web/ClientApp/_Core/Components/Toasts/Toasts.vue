    ...
    <div aria-live="polite" aria-atomic="true" class="toast-container">
        <TransitionGroup name="toast" tag="div" appear>
            <div v-for="item in messages"
                ...
                <div class="toast-header rounded-0" :class="'bg-' + item.type" style="height: 35px;">
                    <strong class="">
                    ...
                    <button v-if="item.closeable"
                            type="button"
                            class="mr-2 mb-1 btn-close"
                            data-dismiss="toast"
                            aria-label="Close"
                            @click="remove(item)">
                ...
                <div v-if="item.timeOut > 0 && item.showProgress"
                     class="progress rounded-bottom m-1"
                     style="height: 6px;">
    ...
    export default {
        ...
        methods: {
            ...
            handleMessageEvent(payload) {
                if (payload.message) {
                    this.addMessage(payload.title, payload.message, payload.type, payload.options);
                }
            },
             ...
             * @param {string} type
             */
            async addMessage(title, message, type, options = {}) {
                // message object data
                var messageData = {
                    id: this.count++,
                    title: title,
                ...
                const start = async () => {
                    ...
                    messageData.animationFrame = requestAnimationFrame(timestamp => {
                        ...
                        if (!self.exists(messageData) || messageData.paused) {
                            ...
                            //  check if this have an onClose callback
                            if (typeof messageData.onClose === 'function') {
                                messageData.onClose();
                            }
