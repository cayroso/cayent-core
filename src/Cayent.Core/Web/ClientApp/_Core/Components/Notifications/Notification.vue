...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //this.$bus.$on("toast-message", this.handleMessageEvent);
        },
        async mounted() {
            const vm = this;
        ...
        methods: {
            async connectNotificationHub() {
                ...
                notificationHub.on('onNotificationCreated', function (resp) {
                    let content = resp.content;
                    
                    if (resp.notificationEntityClassText === 'Announcement') {
                        content += '<br/>';
                        content += `<a href="${vm.urlViewAnnouncement}/${resp.referenceId}">Click ${resp.notificationEntityClassText} here</a>`;
                    }
                    if (resp.notificationEntityClassText === 'Billing') {
                        content += '<br/>';
                        content += `<a href="${vm.urlViewBilling}/${resp.referenceId}">Click ${resp.notificationEntityClassText} here</a>`;
                    }
                    if (resp.notificationEntityClassText === 'Reservation') {
                        content += '<br/>';
                        content += `<a href="${vm.urlViewReservation}/${resp.referenceId}">Click ${resp.notificationEntityClassText} here</a>`;
                    }
                    
                    let op = resp.iconClass;

                    if (op === 'danger')
                        op = 'error';


                    vm.$toast[op](resp.subject, content, { timeOut: 10000 });
